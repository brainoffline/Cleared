...
namespace XAnimations
{
	...
	public class StandUpAnimator : BaseViewAnimator
	{
		protected override void Prepare(UIView view)
		{
			view.Layer.AnchorPoint = new CoreGraphics.CGPoint(0.5, 1);
            PlayTogether(
	...
	public class SwingAnimator : BaseViewAnimator
    {
        ...
        {
            PlayTogether(
                CreateKeyFrame(TransformRotation, 
                               0, DegreesToRadians(10), DegreesToRadians (-10), DegreesToRadians(6), DegreesToRadians (-6), DegreesToRadians(3), DegreesToRadians (-3), 0)
            );
        }
    }
    ...
    public class WaveAnimator : BaseViewAnimator
    {
        protected override void Prepare(UIView view)
        {
            ...
            view.Center = new CoreGraphics.CGPoint(center.X, view.Bounds.Height);
			view.Layer.AnchorPoint = new CoreGraphics.CGPoint(0.5f, 1f);
    ...
    public class WobbleAnimator : BaseViewAnimator
    {
        protected override void Prepare(UIView view)
        {
			...
			var center = view.Center;
			view.Center = new CoreGraphics.CGPoint(center.X, 0f);
			view.Layer.AnchorPoint = new CoreGraphics.CGPoint(0.5f, 0f);
            PlayTogether(
                CreateKeyFrame(TranslationX, CAMediaTimingFunction.EaseInEaseOut, 0 * one, -25 * one, 20 * one, -15 * one, 10 * one, -5 * one, 0 * one, 0), 
                CreateKeyFrame(TransformRotation, 0, DegreesToRadians(-5), DegreesToRadians(3), DegreesToRadians(-3), DegreesToRadians(2), DegreesToRadians(-1), 0, 0));
        }
    }
